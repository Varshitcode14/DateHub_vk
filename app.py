...
class RateLimiter:
    ...
    def allow(self, key):
        now = time.time()
        self.tokens[key] = [t for t in self.tokens[key] if t > now - self.per]
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://new_datehub_db_user:5SmRPdpXSjEWixAgbHr7vKoyvfvwTQdd@dpg-cv8r76bqf0us73bcdr70-a.oregon-postgres.render.com/new_datehub_db'
...
cloudinary.config(
    cloud_name="dojylptlg",
    api_key="914999718546788",
    api_secret="XzDtqMIJaB-mbkEeVfR040LUVIw"
)
...
class User(UserMixin, db.Model):
    ...
    image_url = db.Column(db.String(255))  # Changed from 'image' to 'image_url'
...
def submit_confession():
    ...
    new_confession = Confession(
        ...
        gender=current_user.gender,  # Always include gender
